...
const Layout = ({onLogout, user}) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
        ...
        const {data} = await axios.get("http://localhost:4000/api/tasks/ms", {
  ...
  const stats = useMemo(() => {
     ...
     const StatCard = ({ tittle, value, icon }) => (
       ...
       <div className='ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300'>
           <div className='grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6'>
              ...
              <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
                  <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-600'>
                       ...
                       <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                          <StatCard tittle='Total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5
                          h-3.5 sm:w-4 sm:h-4 text-purple-500'/>}/> 
                          <StatCard tittle='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5
                          h-3.5 sm:w-4 sm:h-4 text-green-500'/>}/>  
                          <StatCard tittle='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5
                          h-3.5 sm:w-4 sm:h-4 text-fuchsia-500'/>}/>  
                          <StatCard tittle='Completion Rate' value={`${stats.completionPercentage} %`} icon={<Zap className='w-3.5
                          h-3.5 sm:w-4 sm:h-4 text-purple-500'/>}/>       
                       </div>
                       ...
                       <div className='space-y-2 sm:space-y-3'>
                             <div className='flex items-center justify-between text-gray-700'>
                                 ...
                                 <span className='text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5
                                 sm:px-2 rounded-full'>
                             ...
                             <div className='relative pt-1'>
                                <div className='flex gap-1.5 items-center'>
                                    <div className='flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
                                       <div className='h-full bg-linear-to-r from-fuchsia-500 to-purple-600
                  ...
                  <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                       ...
                       <div className='space-y-2 sm:space-y-3'>
                          {tasks.slice(0,3).map((task) => (
                            ...
                            justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors
                            duration-200 border border-transparent hover:border-purple-100'>
                               <div className='flex-1 min-w-0'>
                                   <p className='text-sm font-medium text-gray-700 wrap-break-words whitespace-normal'>
                                       {task.tittle}
                                   </p>
                               ...
                               ml-2 ${task.completed ? 'bg-green-100 text-green-700'
