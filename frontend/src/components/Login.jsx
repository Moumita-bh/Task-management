...
const Login = ({onSubmit, onSwitchMode}) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if(!rememberMe){
      toast.error('You must unable remember me to login.')
      return
    }
    setLoading(true)
    ...
    try {
      ...
      toast.success("Login sucfessful! Redirecting...")
  ...
  return (
    <div className='max-w-md bg-white w-full shadow-lg border-purple-100 rounded-xl p-8'>
        ...
        <div className='mb-6 text-center'>
            <div className='w-16 h-16 bg-linear-to-br from-fuchsia-500 to-purple-600 rounded-full
            ...
            <p className='text-gray-500 text-sm mt-1'>Sign to continue to TaskManager</p>
        </div> 

        <form onSubmit={handleSubmit} className='space-y-4'>
             {FIELDS
            .filter(field => field.name === "email" || field.name === "password")
             .map(({ name, type, placeholder, icon: Icon, isPassword }) => (
          ...
          />
           ...
           <div className='flex items-center'>
                 ...
                 className='ml-2 black text-sm text-gray-700'>
                 Remember Me
                 </label>
           </div>
        ...
        <p className='text-center test-sm text-gray-600 mt-6'>
