...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/${task._id}/ms`, { completed: newStatus }, { headers: getAuthHeaders() })
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/${task._id}/ms`, { headers: getAuthHeaders() })
      onRefresh?.()
    } catch (err) {
      console.error(err)
      if (err.response?.status === 401) onLogout?.()
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/${task._id}/ms`, payload, { headers: getAuthHeaders() })
