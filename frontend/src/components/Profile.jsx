...
const changePassword = async (e) => {
     ...
     try {
        ...
        if(data.success){
            ...
            setPasswords({current: "", new: "", confirmed: ""})
        }
        else toast.error(data.message)
     } catch (error) {
        toast.error(
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='max-w-4xl mx-auto p-6'>
            ...
            <div className='flex items-center gap-4 mb-8'>
                <div className='w-16 h-16 rounded-full bg-linear-to-br from-fuchsia-500 to-purple-600
                flex items-center justify-center text-white text-2xl font-bold shadow-md'>
                    {profile.name? profile.name[0].toUpperCase() : "U"}   
                </div>
                <div>
                    <h1 className='text-3xl font-bold text-gray-800'>Account Settings</h1>
                    <p className='text-gray-500 text-sm'>Manage your and security Settings</p>
                </div>
            </div>
                    ...
                    <form onSubmit={saveProfile} className='space-y-4'>
                        {personalFields.map(({name, type, placeholder, icon: Icon}) => (
                            <div key={name} className={INPUT_WRAPPER}>
                                             ...
                                             <input type={type} placeholder={placeholder} value={profile[name]} onChange={
                                                (e) => setProfile({...profile, [name]: e.target.value})}
                    ...
                    <form onSubmit={changePassword} className='space-y-4'>
                        {securityFields.map(({name, placeholder}) => (
                            <div key={name} className={INPUT_WRAPPER}>
                                             ...
                                             <input type="password" placeholder={placeholder} value={passwords[name]} onChange={
                                                (e) => setPasswords({...passwords, [name]: e.target.value})}
                                                className='w-full focus:outline-none text-sm' required
                                            />
                                          </div>
                        ))}
